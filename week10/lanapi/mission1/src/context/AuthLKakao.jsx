...
// AuthKakao.jsx의 fetchKakaoUserInfo 함수에서
export const fetchKakaoUserInfo = async (accessToken, navigate) => {
  try {
      const response = await axios.get('https://kapi.kakao.com/v2/user/me', {
          headers: {
      ...
      const userInfo = {
          kakaoId: id,
          nickname: properties?.nickname || kakao_account?.profile?.nickname,
          loginType: 'kakao'  // 이 부분이 중요합니다
      };

      console.log('저장할 유저 정보:', userInfo);
      localStorage.setItem('user', JSON.stringify(userInfo));
      
      // AuthContext의 상태를 직접 업데이트하기 위해 window.location.href 대신
      // navigate를 사용하고 상태를 업데이트합니다
      if (window.authContext && window.authContext.login) {
          window.authContext.login(userInfo);
      }
      navigate('/');
  } catch (error) {
