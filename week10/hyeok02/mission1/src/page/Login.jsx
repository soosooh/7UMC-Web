...
const formContainerStyles = { width: '300px', margin: '0 auto', marginTop: '200px', textAlign: 'center' };
...
const CustomInput = ({ label, type, register, error }) => (
  <div style={{ height: '55px', marginBottom: '15px' }}>
    <input
      ...
      {...register}
      placeholder={label}
      style={{ ...inputStyles, border: error ? '1px solid red' : '1px solid #ccc' }}
    />
...
function LoginPage({ setUserEmail }) {
  ...
  const mutation = useMutation(
    (data) => axiosInstance.post('/auth/login', data),
    {
      onSuccess: (response) => {
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <CustomInput
          ...
          register={register('password', {
            ...
            maxLength: { value: 16, message: '비밀번호는 16자리 이하로 입력하세요.' },
          })}
          error={errors.password}
        />
        <button type="submit" disabled={!isValid || mutation.isLoading} style={buttonStyles(isValid)}>
