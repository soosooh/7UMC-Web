...
// 수정된 signUpSchema: 닉네임과 생년월일 필드 제거
...
function SignUp() {
  ...
  const onSubmit = async (data) => {
    const requestData = {
      ...
      passwordCheck: data.passwordConfirm, // 필드 이름 변경
    };
    console.log('보내는 데이터:', requestData); // 보내는 데이터 확인
    try {
      const response = await axiosInstance.post('/auth/register', requestData);
      console.log('회원가입 성공:', response.data); // 성공 응답 확인
      navigate('/login');
    } catch (error) {
      console.error('회원가입 실패:', error.response ? error.response.data : error.message); // 에러 응답 확인
    }
  };
